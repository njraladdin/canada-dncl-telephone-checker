...
NUM_BROWSERS = 10  # Number of browser instances to run
...
class ResultState:
    ...
    def print_progress(self):
        print(f"\n=== PROGRESS UPDATE (Completed: {self.total_attempts}/{TOTAL_ATTEMPTS}) ===")
...
def random_delay(min_ms=10, max_ms=50):
    delay = random.randint(min_ms, max_ms) / 1000.0
    time.sleep(delay)

def download_audio(url, max_attempts=3):
    headers = {
...
def solve_audio_challenge(challenge_frame):
    try:
...
def wait_for_recaptcha_frame(tab, timeout=10):
...
def wait_for_checkbox_clickable(iframe, timeout=10):
...
def process_single_tab(browser, results_queue):
    try:
        # Get new tab
        tab = browser.new_tab()
        ...
        frame_info = wait_for_recaptcha_frame(tab)
        if not frame_info:
        ...
        try:
            ...
            while time.time() - start_time < 7:
                ...
                if token:
                    print("Token received:", token[:50] + "..." if len(token) > 50 else token)
                ...
                if challenge_info.get('found'):
                    ...
                    if challenge_frame:
                        if solve_audio_challenge(challenge_frame):
                            solve_start_time = time.time()
                            while time.time() - solve_start_time < 10:
                                token = tab.run_js(js_check_token)
                                if token:
                                    print("Token received after audio challenge:", token[:50] + "..." if len(token) > 50 else token)
                                    results_queue.put((True, token))
                                    return
                                time.sleep(0.5)
...
def check_phone_number():
    try:
        ...
        for _ in range(TABS_PER_BROWSER):
            thread = threading.Thread(target=process_single_tab, args=(browser, results_queue))
            thread.start()
        ...
        for _ in range(TABS_PER_BROWSER):
            ...
            if success:
                ...
                results.failed.append(datetime.now())
...
# Main execution
print(f"\nStarting execution with {NUM_BROWSERS} browsers, {TABS_PER_BROWSER} tabs each ({TOTAL_ATTEMPTS} total attempts)")
print("=" * 50)

for i in range(NUM_BROWSERS):
    print(f"\nStarting Browser {i + 1}/{NUM_BROWSERS}")
    print("-" * 50)
    check_phone_number()
    time.sleep(1)
...
print("\nFinal Results:")
print("=" * 50)
print(f"Total attempts completed: {results.total_attempts}")
print(f"Success rate: {results.success_rate:.1f}%")
print(f"Total successful tokens: {len(results.successful)}")
