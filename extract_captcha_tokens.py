...
class CaptchaTokenExtractor:
    ...
    def extract_tokens(self, attempts=None):
        """
        Main method to extract captcha tokens
        attempts: Optional number of total attempts to make (overrides num_browsers * tabs_per_browser)
        """
        if attempts:
            self.total_attempts = attempts

        print(f"\nStarting execution with {self.tabs_per_browser} tabs (max {self.total_attempts} attempts)")
        print("=" * 50)
